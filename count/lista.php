...
<?php 
    usort($simulacoesCount, function($a, $b) {
...
?>
	...
	<h2 class="text-center">Lista Contadores</h2>
	<div class="row">
		<div class="col">
			<?php if(count($simulacoesCount) > 0): ?>
			<table class="table">
                <thead>
                    <tr>
                        <th>COD</th>
                ...
                <tbody>
				<?php foreach($simulacoesCount as $s): 
                    $dt = isset($s['dataSimulacao']) ? (new DateTime($s['dataSimulacao']))->format('d/m/y H:i:s') : '-';     
                ?>
		  		<tr>
                    <td>#<?= $s['form']['codCount'] ?></td>
                    <td style="max-width: 200px; overflow: hidden;" title="<?=json_encode($s['form'])?>">
                        <?= json_encode($s['form']) ?>
                    </td>
                    <td><?= $dt ?></td>
                    <td><a class="ml-3" href="contador.php?codCount=<?= $s['form']['codCount'] ?>">Ver</a>
					<a class="ml-3" href="excluir.php?codCount=<?= $s['form']['codCount'] ?>">Excluir</a></td>
                </tr>
				<?php endforeach; ?>
                </tbody>
			</table>
